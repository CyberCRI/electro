...
async def validate_audio_file(file: UploadFile) -> None:
    ...
    if file.size and file.size > settings.MAX_AUDIO_FILE_SIZE:
        raise HTTPException(
            status_code=413, detail=f"File too large. Maximum size: {settings.MAX_AUDIO_FILE_SIZE // (1024 * 1024)}MB"
        )


async def transcribe_audio(
    file: UploadFile, language: Optional[str] = None, response_format: str = "json", temperature: float = 0.0
) -> dict:
    """
    ...
    client = AsyncOpenAI(base_url=settings.OPENAI_API_BASE_URL, api_key=settings.OPENAI_API_KEY)
